...
exten => _X.,1,NoOp(PJSIP MESSAGE to \${EXTEN} from \${MESSAGE(from)})
 ...
 ; store → id ; 0 = accusé JSON (chat_delivered/read) : forward sans file pending
 ...
 ; SUCCESS → mark delivered + push SIP chat_delivered à l’émetteur (ticks ✓✓ immédiats)
 same => n,ExecIf(\$["\${MESSAGE_SEND_STATUS}"="SUCCESS" & \${LEN(\${CHAT_ID})} > 0 & "\${CHAT_ID}" != "0"]?System(sudo -n /usr/local/bin/asaphone-chat-ingest delivered \${CHAT_ID} 2>/dev/null))
 same => n,System(rm -f \${CHAT_TMP} \${CHAT_TMP}.from \${CHAT_TMP}.id)
 same => n,Hangup()

[vm-notify-out]
; EXTEN = 1005.token (unique) ou 1005 (legacy)
exten => _X.,1,NoOp(Notification SIP → \${EXTEN})
 same => n,Set(REAL_EXT=\${CUT(EXTEN,.,1)})
 same => n,Set(NOTIFY_TOKEN=\${CUT(EXTEN,.,2)})
 same => n,GotoIf(\$["\${LEN(\${NOTIFY_TOKEN})}" = "0"]?legacy)
 same => n,Set(VM_NOTIFY_FILE=${NOTIFY_DIR}/vm-notify-\${REAL_EXT}-\${NOTIFY_TOKEN}.json)
 same => n,Set(VM_CALLER_FILE=${NOTIFY_DIR}/vm-notify-\${REAL_EXT}-\${NOTIFY_TOKEN}.caller)
 same => n,Goto(send)
 same => n(legacy),Set(REAL_EXT=\${EXTEN})
 same => n,Set(VM_NOTIFY_FILE=${NOTIFY_DIR}/vm-notify-\${EXTEN}.json)
 same => n,Set(VM_CALLER_FILE=${NOTIFY_DIR}/vm-notify-\${EXTEN}.caller)
 same => n(send),Set(VM_NOTIFY_BODY=\${SHELL(cat \${VM_NOTIFY_FILE} 2>/dev/null | tr -d '\\n')})
 same => n,GotoIf(\$["\${LEN(\${VM_NOTIFY_BODY})}" = "0"]?fail)
 same => n,Set(VM_CALLER=\${SHELL(cat \${VM_CALLER_FILE} 2>/dev/null | tr -d '\\n')})
 same => n,Set(VM_CALLER=\${IF(\$[\${LEN(\${VM_CALLER})} > 0]?\${VM_CALLER}:pbx)})
 same => n,Set(MESSAGE(from)=sip:\${VM_CALLER}@${PBX_DOMAIN})
 same => n,Set(MESSAGE(to)=sip:\${REAL_EXT}@${PBX_DOMAIN})
 same => n,Set(MESSAGE(body)=\${VM_NOTIFY_BODY})
 same => n,MessageSend(pjsip:\${REAL_EXT})
 same => n,System(rm -f \${VM_NOTIFY_FILE} \${VM_CALLER_FILE})
 same => n,Hangup()
